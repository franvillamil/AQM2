...
\begin{tabular}{p{2.3cm}p{3.2cm}p{4.5cm}}
\hline
...
\begin{itemize}
\item You do not pick a design, \textit{the data} pick one for you
\item What source of exogenous variation does the world hand you?
\end{itemize}
...
% ----------------------------------------------------
\begin{frame}
\frametitle{What's next (not in this class)}
\centering

\begin{itemize}
\item \textbf{Multilevel / hierarchical models} (Gelman \& Hill)
  \begin{itemize}
  \item Natural framework when data are nested in groups
  \end{itemize}
\vspace{6pt}
\item \textbf{Bayesian inference}
  \begin{itemize}
  \item Priors, posteriors, and what MCMC actually does
  \item McElreath, \textit{Statistical Rethinking}
  \end{itemize}
\vspace{6pt}
\item \textbf{Machine learning for social science}
  \begin{itemize}
  \item Prediction-first tools: lasso, random forests, boosting
  \item Causal ML (honest trees, double/debiased ML)
  \end{itemize}
\vspace{6pt}
\item \textbf{Text as data}
  \begin{itemize}
  \item From bag-of-words to LLMs in social science
  \end{itemize}
\end{itemize}

\end{frame}
% ----------------------------------------------------
\note{Pointers rather than content. I will put a bit more on multilevel models and text as data in the appendix of these slides; the others are entire courses on their own, but the references listed here are the standard starting points. The McElreath book in particular is the most approachable introduction to Bayesian thinking aimed at social scientists.}
...
\item What is the \textbf{DAG} behind the relationship?
\item What method matches the question and the variation?
\item What are the \textbf{identifying assumptions}, and how plausible are they?
\item What \textbf{alternative explanations} does the design \textit{not} rule out?
...
{\Large Good luck with the essay.}

\vspace{30pt}

{\small Appendix follows: multilevel models and text as data.\\Self-study material, not covered in class.}
