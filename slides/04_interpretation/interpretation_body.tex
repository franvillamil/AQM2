...
\item In R:
  ...
  \item \texttt{predictions(model, newdata = datagrid(age = 35, college = 1))}
  \end{itemize}
\end{itemize}
...
\item \texttt{plot\_predictions(model, condition = "age")}
  ...
  \item Includes 95\% confidence band
  \item Holds other variables at their means (or modes)
  \end{itemize}
\vspace{8pt}
\item With two conditions:
  \begin{itemize}
  \item \texttt{plot\_predictions(model, condition = c("age", "female"))}
...
\item In R:
  \begin{itemize}
  \item \texttt{comparisons(model, variables = list(college = c(0, 1)))}
...
\item Standard approach: delta method (what marginaleffects uses)
\vspace{8pt}
...
\begin{tikzpicture}[
  ...
  % Step 2
  \node[box, fill=accent!10] (draw) at (0,1.2) {Draw $\tilde{\beta}^{(s)}$ from\\$N(\hat{\beta}, \hat{V})$, $s = 1, \ldots, S$};
  % Step 3
  \node[box, fill=accent!10] (compute) at (0,-0.6) {For each draw, compute\\quantity of interest $Q^{(s)}$};
  % Step 4
  \node[box, fill=accent2!10] (result) at (0,-2.4) {Use distribution of\\$Q^{(1)}, \ldots, Q^{(S)}$};
  % Arrows
  \draw[arrow] (est) -- (draw);
  ...
  \node[right, font=\scriptsize, asher] at (2.2,1.2) {\texttt{sim\_betas = MASS::mvrnorm(1000, coef(m), vcov(m))}};
  \node[right, font=\scriptsize, asher] at (2.2,-0.6) {\texttt{sim\_pred = sim\_betas \%*\% x\_scenario}};
  \node[right, font=\scriptsize, asher] at (2.2,-2.4) {\texttt{quantile(sim\_pred, c(0.025, 0.975))}};
\end{tikzpicture}
...
\item[] \texttt{predictions(m, newdata = datagrid(age = 40, education = 16, female = 1))}
\item[]
...
\item Predicted values, marginal effects, and first differences are your tools
\item The \texttt{marginaleffects} package handles everything
\vspace{8pt}
\item Present results with plots (prediction plots, coefficient plots)
\item Tables go in the appendix, plots in the main text
\vspace{8pt}
\item Always check diagnostics: residual plots, heteroskedasticity
\item Always use robust standard errors
\item Use log transformations for skewed variables (but interpret carefully)
\end{itemize}
