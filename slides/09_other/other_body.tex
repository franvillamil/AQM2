...
\begin{itemize}
\item Presentations (April 16th)
  \begin{itemize}
    \item {\color{red}{\textbf{5-minute}}} \textbf{presentations}, blocks of $\approx$5, random order,
    \item[] 15-min feedback after each block
    \item[] \url{https://franvillamil.github.io/AQM2/logistics.html}
    \item Send me \textbf{presentations} by email \textbf{in advance}
    \item[] \textbf{3 slides max!:} e.g. what, how, problems
  \end{itemize}
\item Rest of the course
  \begin{itemize}
      \item Computing session (April 23rd)
      \item Final session (April 30th), \textbf{exam} + review
  \end{itemize}
\end{itemize}
...
Nominal ($>$2) & Multinomial & Log-odds & Multinom.\ logit (\texttt{multinom()}) \\[2pt]
Count ($\geq 0$) & Poisson & Log & Poisson (\texttt{glm(...,poisson)}) \\[2pt]
Count (overdisp) & Neg.\ Binomial & Log & NegBin (\texttt{glm.nb()}) \\[2pt]
...
% ----------------------------------------------------
\begin{frame}[fragile]
\frametitle{Ordered logit: R output}
\centering

\vspace{2pt}
{\scriptsize Using \texttt{housing} data from MASS: satisfaction with housing conditions}

\vspace{4pt}

{\fontsize{4.5pt}{5.5pt}\selectfont
\begin{verbatim}
...
\begin{itemize}
\item {\small \textbf{Coefficients}: log-odds scale, not directly interpretable}
\item {\small \textbf{Intercepts}: estimated thresholds $\tau_j$ between categories}
\item {\small No $p$-values --- use $t$-values $> 2$ as rough guide}
\end{itemize}

\end{frame}
% ----------------------------------------------------
\note{Walk through each section of the output. The \textbf{Coefficients} table shows the effect of each predictor on the latent variable in log-odds units. The sign tells us direction: positive means higher values of the predictor shift the distribution toward higher categories. For example, \texttt{InflHigh} (high perceived influence) has a coefficient of 1.29, meaning people who feel they have high influence over housing management are much more likely to report high satisfaction. The \textbf{Intercepts} (thresholds) define the cut points between categories on the latent scale. There are $K-1 = 2$ thresholds for 3 categories (Low, Medium, High satisfaction). Note: \texttt{polr()} does not report $p$-values directly. The $t$-values follow an approximate normal distribution in large samples, so $|t| > 2$ is roughly significant at 5\%. For formal inference, use \texttt{avg\_slopes()}.}

% ----------------------------------------------------
\begin{frame}[fragile]
\frametitle{Ordered logit: marginal effects}
\centering

\vspace{2pt}
{\scriptsize\texttt{avg\_slopes(m\_ord)} --- average marginal effects on probability scale}
...
\begin{itemize}
\item {\small One row per predictor $\times$ outcome category}
\item {\small \textbf{Group}: which satisfaction level (Low, Medium, High)}
\item {\small E.g., high influence $\to$ +29pp probability of ``High'' satisfaction}
\item {\small AMEs across categories \textbf{sum to zero} for each predictor}
\end{itemize}

\end{frame}
% ----------------------------------------------------
\note{This is the key interpretive output. Each row gives the average marginal effect of a predictor on the probability of a specific outcome category. Read the \texttt{Group} column carefully: it tells you which satisfaction category the effect refers to. For example, having high perceived influence (vs.\ low) increases the probability of being in the ``High'' satisfaction group by about 29 percentage points, while decreasing the probability of being in the ``Low'' group by about 26 percentage points. The AMEs across all categories must sum to zero for each predictor --- probability shifted out of one category must go into another. This is a good sanity check. Note: the output is trimmed here for the slide; the full output has one row per predictor per category per contrast.}
...
% ----------------------------------------------------
\begin{frame}
\frametitle{Application: Markov transition models}
\centering

\vspace{6pt}

\begin{itemize}[<+->]
\item Ordinal models can be used to study \textbf{transitions between ordered states}
  \begin{itemize}
  \item Democracy scores (autocracy $\to$ anocracy $\to$ democracy)
  \item Credit ratings (AAA $\to$ AA $\to$ A $\to$ \ldots)
  \item Health status (healthy $\to$ mild $\to$ severe $\to$ critical)
  \end{itemize}
\vspace{8pt}
\item \textbf{Idea}: given current state $S_t$, model the probability of transitioning to each state at $S_{t+1}$ using ordinal logit
\vspace{8pt}
\item The ordinal structure constrains transitions to respect the natural ordering --- large jumps (e.g., autocracy $\to$ full democracy) are possible but predicted to be rare
\vspace{8pt}
\item Covariates shift the latent propensity, making upward or downward transitions more likely
  \begin{itemize}
  \item E.g., economic crisis $\to$ higher probability of moving down the scale
  \end{itemize}
\vspace{8pt}
\item In practice: \textbf{interact all covariates with $S_t$}
  \begin{itemize}
  \item Allows the effect of each predictor to differ by current state
  \item Equivalent to fitting a separate ordinal model per origin state
  \end{itemize}
\end{itemize}

\end{frame}
% ----------------------------------------------------
\note{This slide shows students one application of ordinal models beyond the typical survey-item setting. A Markov transition model asks: given that a unit is in state $j$ at time $t$, what is the probability it moves to each possible state at $t+1$? If the states are ordered, an ordinal logit is a natural fit --- it models the transition probabilities while respecting the ordering constraint. For example, Epstein et al.\ (2006) model transitions between political regime types (autocracy, partial democracy, full democracy) as an ordered outcome. The ordered logit ensures that jumping two categories is less probable than moving one step, which matches our substantive expectation. The last point is important in practice: by interacting all covariates with the current state $S_t$, you allow the effects to be state-dependent --- e.g., the effect of an economic crisis on regime change might differ depending on whether the country is already an autocracy or a partial democracy. This is equivalent to subsetting the data by current state and fitting separate ordinal models, but the interaction approach keeps everything in one model and allows you to test whether the differences across origin states are significant.}
...
% ----------------------------------------------------
\begin{frame}[fragile]
\frametitle{Multinomial logit: R output}
\centering

\vspace{2pt}
{\scriptsize Using \texttt{iris} data: predicting species from flower measurements}
...
\begin{itemize}
\item {\small One row of coefficients \textbf{per non-reference category}}
\item {\small Reference = \textit{setosa}; each row = log-odds vs.\ reference}
\item {\small Coefficients are log-odds $\to$ hard to interpret directly}
\item {\small Use \texttt{avg\_slopes()} or \texttt{plot\_predictions()} instead}
\end{itemize}

\end{frame}
% ----------------------------------------------------
\note{Walk through the output structure. The key difference from binary logit: there is one row of coefficients for each non-reference category. Here the reference category is \textit{setosa} (the first level), so we get one row for \textit{versicolor} (log-odds of versicolor vs.\ setosa) and one for \textit{virginica} (log-odds of virginica vs.\ setosa). Each coefficient gives the change in log-odds of that category relative to the reference for a one-unit change in the predictor. For example, Petal.Length has a coefficient of 15.7 for versicolor and 28.9 for virginica --- longer petals strongly increase the odds of both categories relative to setosa, but more so for virginica. The large standard errors in this example reflect near-perfect separation. The raw coefficients are rarely reported; always interpret via marginal effects or predicted probabilities.}

% ----------------------------------------------------
\begin{frame}[fragile]
\frametitle{Multinomial logit: marginal effects}
\centering

\vspace{2pt}
{\scriptsize\texttt{avg\_slopes(m\_mnom)} --- average marginal effects on probability scale}
...
\textbf{Independence of Irrelevant Alternatives (IIA):} odds ratio btw any two alternatives \textbf{unaffected} by presence of other alternatives
...
% ----------------------------------------------------
\begin{frame}[fragile]
\frametitle{Poisson: R output}
\centering

\vspace{2pt}
{\scriptsize Using \texttt{warpbreaks}: number of breaks in yarn by wool type and tension}
...
\begin{itemize}
\item {\small Coefficients on \textbf{log scale} $\to$ exponentiate for IRRs}
\item {\small \texttt{woolB}: $\exp(-0.21) = 0.81$ $\to$ 19\% fewer breaks}
\item {\small \alert{Red flag}: residual deviance (210) $\gg$ df (50) $\to$ overdispersion!}
\end{itemize}

\end{frame}
% ----------------------------------------------------
\note{Walk through the output top to bottom. The coefficients table looks like any GLM output: estimates, standard errors, $z$-values, $p$-values. The key interpretive step is exponentiation. \texttt{woolB = -0.206}: $\exp(-0.206) = 0.814$, meaning wool type B has about 19\% fewer breaks than wool type A (the reference). \texttt{tensionH = -0.519}: $\exp(-0.519) = 0.595$, meaning high tension reduces breaks by about 40\% compared to low tension. The line ``Dispersion parameter for poisson family taken to be 1'' is important: Poisson \emph{assumes} dispersion equals 1. But look at the deviance: residual deviance is 210 on 50 df --- a ratio of 4.2, far above 1. This is a clear sign of overdispersion. The standard errors are likely too small, and we should consider a negative binomial model instead.}
...
% ----------------------------------------------------
\begin{frame}[fragile]
\frametitle{Negative binomial: R output}
\centering

\vspace{2pt}
{\scriptsize Same \texttt{warpbreaks} data --- compare with Poisson output}
...
\begin{itemize}
\item {\small \textbf{SEs are larger} than Poisson (e.g., \texttt{woolB}: 0.10 vs.\ 0.05)}
\item {\small \texttt{woolB} now only marginal ($p = 0.065$) --- was ``highly significant''}
\item {\small Deviance/df $\approx 1.07$ --- overdispersion is handled}
\end{itemize}

\end{frame}
% ----------------------------------------------------
\note{This is the key comparison slide. Use the same data as the Poisson model so students can see the differences directly. Three things to highlight: (1) The coefficient \emph{estimates} are similar (NegBin: $-0.186$ vs.\ Poisson: $-0.206$ for \texttt{woolB}), but (2) the \emph{standard errors} are roughly doubled (0.10 vs.\ 0.05). This is because the negative binomial correctly accounts for the extra-Poisson variation. As a result, \texttt{woolB} goes from highly significant ($p < 0.001$) to only marginally significant ($p = 0.065$). The Poisson model was overconfident. (3) The residual deviance is now 53.7 on 50 df (ratio $\approx 1.07$), indicating the model fits well. The Theta parameter ($\hat\theta = 9.94$) is the estimated dispersion --- not huge but clearly needed. The AIC is also much lower (409 vs.\ 493), confirming the NegBin fits better.}
...
\begin{itemize}[<+->]
\item \textbf{Default}: start Poisson, test overdispersion, switch to NegBin if needed (\textbf{in practice}: often go ahead to NB )
...
% ----------------------------------------------------
\begin{frame}
\frametitle{What does survival data look like?}
\centering
...
\begin{tabular}{llccl}
\hline
\textbf{Unit} & \textbf{Spell start} & \textbf{Duration} & \textbf{Event} & \textbf{Covariates \ldots} \\
\hline
Country A & 1975 & 12 years & 1 (breakdown) & GDP, region, \ldots \\
Country B & 1991 & 30 years & 0 (censored)  & GDP, region, \ldots \\
Country C & 1985 &  8 years & 1 (breakdown) & GDP, region, \ldots \\
Country D & 2002 & 19 years & 0 (censored)  & GDP, region, \ldots \\
Country E & 1960 &  3 years & 1 (breakdown) & GDP, region, \ldots \\
\hline
\end{tabular}
}
...
\begin{itemize}[<+->]
\item Each row is a \textbf{spell}: one unit observed from entry to event or censoring
\item Two outcome variables: \textbf{duration} ($T$) and \textbf{event indicator} ($d$: 1 = event, 0 = censored)
\item In R: combined into a \texttt{Surv(time, event)} object
\end{itemize}

\end{frame}
% ----------------------------------------------------
\note{This slide shows students the concrete data structure for survival analysis before any formulas. Each row is a spell --- a period of time during which a unit is at risk of the event. The two key columns are: (1) duration, how long the spell lasted, and (2) an event indicator that tells you whether the spell ended with the event (1) or was censored (0). Censored means the observation window closed before the event occurred --- the unit is still ``alive.'' Covariates can be time-invariant (region, regime type at entry) or time-varying (GDP, which updates each year). In R, \texttt{Surv(time, event)} packages these two columns into a single object that all survival functions understand. Connect this to the diagram on the previous slide: Country A and C are the spells ending with filled circles (events), while B and D are the ones hitting the dashed line (censored).}
...
\begin{itemize}
\item[] \texttt{library(survival); library(broom); library(ggplot2)}
\vspace{4pt}
\item[] \texttt{\# Kaplan-Meier estimator}
\item[] \texttt{km\_fit = survfit(Surv(time, event) \textasciitilde{} group, data = d)}
\item[] \texttt{ggplot(tidy(km\_fit), aes(time, estimate, color = strata)) +}
\item[] \texttt{~~geom\_step() + geom\_ribbon(aes(ymin = conf.low,}
\item[] \texttt{~~ymax = conf.high, fill = strata), alpha = .2)}
\vspace{4pt}
...
\end{frame}
% ----------------------------------------------------
\note{Walk through the syntax. The \texttt{Surv(time, event)} object is the key data structure: \texttt{time} is the duration variable, \texttt{event} is a binary indicator for whether the event occurred (1) or the observation was censored (0). This object is always the left-hand side of the formula. \texttt{survfit()} fits the Kaplan-Meier estimator; \texttt{broom::tidy()} converts the result into a data frame that \texttt{ggplot2} can plot directly using \texttt{geom\_step()} for the survival curve and \texttt{geom\_ribbon()} for confidence bands. \texttt{coxph()} fits the Cox proportional hazards model. The output includes coefficients on the log-hazard scale and, when you run \texttt{summary()}, the exponentiated hazard ratios. Always test the PH assumption with \texttt{cox.zph(cox\_fit)}: a significant test indicates time-varying effects that violate the PH assumption.}

% ----------------------------------------------------
\begin{frame}[fragile]
\frametitle{Cox PH: R output}
\centering

\vspace{2pt}
{\scriptsize Using \texttt{lung} data (survival pkg): survival of lung cancer patients}
...
\begin{itemize}
\item {\small \texttt{exp(coef)} = \textbf{hazard ratios} --- the key quantity to report}
\item {\small \texttt{sex} (2 = female): HR = 0.58 $\to$ 42\% \textit{lower} hazard (longer survival)}
\item {\small \texttt{ph.ecog}: HR = 1.59 $\to$ 59\% \textit{higher} hazard per unit increase}
\item {\small Concordance = 0.64: moderate predictive accuracy}
\end{itemize}

\end{frame}
% ----------------------------------------------------
\note{Walk through the output carefully. The summary shows two tables: (1) the coefficients on the log-hazard scale with standard errors and $p$-values, and (2) the exponentiated coefficients (hazard ratios) with 95\% confidence intervals. The hazard ratio is the key quantity. \texttt{sex}: the \texttt{lung} dataset codes sex as 1 = male, 2 = female. The hazard ratio of 0.575 means females have about 42\% lower hazard of death --- they survive longer. \texttt{ph.ecog} (ECOG performance score, 0 = fully active to 4 = bedridden): each unit increase in performance score (i.e., worse health) increases the hazard by 59\%. Age is not significant. The Concordance statistic (0.637) measures predictive discrimination --- analogous to AUC for binary outcomes. The three global tests at the bottom test whether the model as a whole is significant. All three agree ($p < 0.001$). Note the line ``n = 227, number of events = 164'' --- 164 deaths were observed, 63 observations were censored (still alive at end of study).}
...
% ----------------------------------------------------
\begin{frame}
\frametitle{What if proportional hazards fails?}
\centering

\vspace{6pt}

\begin{itemize}[<+->]
\item Test with \texttt{cox.zph()}: significant $p$-value $\Rightarrow$ effect changes over time
\vspace{8pt}
\item \textbf{Solutions}:
  \begin{itemize}
  \item \textbf{Stratify}: \texttt{coxph(Surv(t,d) \textasciitilde{} x + strata(z))}\\
    {\small Fits separate baseline hazard per group; useful for control variables}
  \vspace{4pt}
  \item \textbf{Time interaction}: let $\beta$ vary with time\\
    {\small Models the changing effect explicitly}
  \vspace{4pt}
  \item \textbf{Split follow-up}: divide time into periods, fit separate models\\
    {\small Simple but choice of cutpoint can be arbitrary}
  \vspace{4pt}
  \item \textbf{Different model}: parametric (Weibull) or accelerated failure time\\
    {\small Do not require proportional hazards}
  \end{itemize}
\end{itemize}

\end{frame}
% ----------------------------------------------------
\note{This is a reference slide --- students do not need to master these solutions, but should know they exist. The most common approach in practice is stratification: if the PH assumption fails for a control variable (e.g., sex), you stratify by it, which allows each stratum to have its own baseline hazard while still estimating a common coefficient for the variables of interest. You lose the ability to estimate the stratifying variable's effect, but that is acceptable if it is just a control. Time interactions are more involved: they let the coefficient change as a function of time, which directly addresses the violation. Splitting follow-up is the most intuitive but least elegant --- it requires choosing a cutpoint and reduces power. Finally, parametric models (Weibull, log-normal) and accelerated failure time (AFT) models operate on a different scale and do not assume proportional hazards. In an applied course, the key message is: always check PH, and if it fails, stratification is usually the easiest fix.}
...
\begin{tikzpicture}[
  ...
  \node[font=\scriptsize, asher] at (0, -1.1)
