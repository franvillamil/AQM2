...
**a)** `LMerr` tests whether there is spatial dependence in the **error term** ($\lambda \neq 0$ in the SEM). `LMlag` tests whether a **spatially lagged dependent variable** belongs in the model ($\rho \neq 0$ in the SLM). Here, `LMerr` is highly significant (p < 0.001) while `LMlag` is not significant (p $\approx$ 0.80). This already points strongly toward the SEM: there is clear evidence of spatial dependence in the errors, but no evidence that a spatially lagged dependent variable is needed.

**b)** The robust tests (`RLMerr`, `RLMlag`) each control for the presence of the other type of spatial dependence. `RLMerr` remains highly significant (p < 0.001), while `RLMlag` is not significant (p $\approx$ 0.14). Since even the standard `LMlag` was not significant, there is no ambiguity here: the diagnostics clearly favor the SEM over the SLM. The spatial dependence in this application operates through spatially correlated omitted factors (the error channel), not through direct diffusion of life expectancy across borders.
...
**a)** Compute centroids and build a distance-based neighborhood (300 km threshold):
...
With a 300 km distance threshold based on centroids, more countries end up with zero neighbors compared to the queen contiguity matrix. Queen contiguity picks up any shared border point, no matter how small, while the distance-based approach relies on centroid-to-centroid distance. Large countries whose centroids are far from any neighbor's centroid — as well as island nations — are likely to be isolated here. On the other hand, some small, clustered countries (e.g., in Europe or Central America) may gain additional neighbors that they did not have under contiguity alone.

**b)** Row-standardized weights and SEM with distance-based neighbors:
...
The `log_gdp` coefficient and $\hat{\lambda}$ from the distance-based SEM can be compared to the contiguity-based SEM in question 4a. Differences between the two sets of estimates illustrate that spatial models are sensitive to how the neighborhood is defined — the choice of weights matrix is a modelling decision, not purely a data feature. If the estimates are broadly similar, this gives some reassurance about robustness; if they diverge, it flags that the spatial structure assumed by the model matters for substantive conclusions.

**c)** Moran's I on the distance-based SEM residuals:
...
As with the contiguity-based SEM, we check whether the distance-based SEM has successfully absorbed the spatial autocorrelation. If Moran's I on these residuals is non-significant, the model has done its job under this alternative definition of neighborhood as well.
