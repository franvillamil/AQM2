\input{preamble.tex}
\textbf{\large Assignment 5: Best Practices in Computing}\\\vspace{10pt}
\end{center}
...
\setlength\itemsep{0pt}
  ...
  \item Submit via your GitHub repository in a separate folder (\texttt{assignment5})
...
Create a new folder \texttt{assignment5} in your course repository with the following structure:
...
Create the file \code{analysis/models.R} with the following structure. The script should be self-contained: anyone should be able to run it from the \texttt{assignment5} folder and reproduce the results.
...
Create a \code{Makefile} in the \texttt{assignment5} folder that automates the pipeline. Recall from the lecture that a Makefile specifies targets, dependencies, and recipes.
...
analysis/output/table_models.tex: analysis/models.R \
  ...
  \item Test it by running \code{make} in the terminal from the \texttt{assignment5} folder. In a comment in your \code{README.md}, note whether it worked and what output was produced.
\end{enumerate}
...
Rewrite this script and save it as \code{assignment5/improved\_script.R}. Your improved version should include:
...
\begin{enumerate}[label=\alph*)]
  \item Make sure your \texttt{assignment5} folder is tracked by Git (it should be part of your course repository).
  ...
  \begin{itemize}
    \item \code{Add folder structure and README for assignment 5}
    \item \code{Create analysis script with regression models}
    \item \code{Add Makefile to automate pipeline}
  \end{itemize}
  \item Create a \code{.gitignore} file inside \texttt{assignment5/} that ignores generated and system files:
  \begin{lstlisting}
*.pdf
*.aux
*.log
...
Commit your work to your GitHub repository before the deadline. Everything should be inside the \texttt{assignment5} folder. Make sure your repository is public so I can access it.

Your \texttt{assignment5} folder should contain:
\begin{itemize}
...
All R scripts should run without errors from the \texttt{assignment5} directory.
