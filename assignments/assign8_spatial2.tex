...
% --------------------------------------------------------------------------
\subsection{Distance-based weights: an alternative neighborhood}
% --------------------------------------------------------------------------
...
\begin{enumerate}[label=\alph*)]
  ...
  \item Compute the centroids of every country and build a distance-based neighborhood in which two countries are neighbors if their centroids are within 300\,km of each other. Use the following code:
  \begin{lstlisting}
coords = st_centroid(st_geometry(world))
nb_dist = dnearneigh(coords, d1 = 0, d2 = 300)
summary(nb_dist)
  \end{lstlisting}
  \textbf{Note on distances and projections.} Earlier in the course we said that computing distances requires projecting to a planar CRS (e.g.\ UTM). That advice applies when you work within a limited area where a single projection is accurate. Here we have a global dataset: no single planar projection preserves distances everywhere on Earth. The function \code{dnearneigh()} handles this automatically --- when it receives an \code{sf} object with a geographic CRS (WGS84), it computes great-circle distances on the ellipsoid, which are accurate worldwide. The 300\,km threshold is therefore interpreted in kilometers without needing to reproject.
  ...
  \item Create row-standardized weights from the distance-based neighborhood and fit a SEM using the same formula (\code{lifeExp \textasciitilde{} log\_gdp}):
  \begin{lstlisting}
...
summary(sem_dist)
  \end{lstlisting}
  In a comment, report $\hat{\lambda}$ and its p-value. Compare the \code{log\_gdp} coefficient and $\hat{\lambda}$ from this model to the contiguity-based SEM in question 1.4a. Are the results substantially different? What does this tell you about the sensitivity of spatial models to the definition of the neighborhood?

  \item Run Moran's I on the residuals of this distance-based SEM (using \code{listw\_dist}). In a comment, does this model also succeed in removing spatial autocorrelation from the residuals? Compare to your answer in 1.4c.
