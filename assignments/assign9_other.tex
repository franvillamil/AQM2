...
summary(m_ologit)
  \end{lstlisting}
  In a comment, report the raw coefficient on \code{Europe} and its sign. Based on the sign convention, does higher support for European integration predict more optimistic views of the national economy?
...
\begin{enumerate}[label=\alph*)]
  ...
  \item Explore the outcome variable \code{art}:
  \begin{lstlisting}
...
dev.off()
  ...
  \item Fit a Poisson regression of \code{art} on all predictors:
  \begin{lstlisting}
...
\begin{enumerate}[label=\alph*)]
  ...
  \item Fit the negative binomial model with the same formula:
  \begin{lstlisting}
m_nb = glm.nb(art ~ fem + mar + kid5 + phd + ment,
              data = bioChemists)
summary(m_nb)
  \end{lstlisting}
  In a comment, compare the coefficient on \code{ment} to the Poisson estimate from the Poisson model above. Has it changed substantially? Report the estimated overdispersion parameter \code{theta} from the NB output. Is the overdispersion modest or severe?
...
Load the required packages (\code{survival}, \code{broom}, \code{ggplot2}, \code{marginaleffects}) and the \code{lung} dataset from \code{survival}.
...
\textbf{Note:} The \code{status} variable uses 1 = censored and 2 = dead, but the \code{Surv()} function expects 0 = censored and 1 = event. Recode it before fitting any models (e.g.\ \code{lung\$dead = lung\$status - 1}).
...
\begin{enumerate}[label=\alph*)]
  ...
  \item Estimate the overall Kaplan-Meier survival curve using \code{survfit()} with formula \code{Surv(time, dead) \textasciitilde{} 1}. The \code{summary()} output shows, at each event time, the number at risk, the number of events, the estimated survival probability, and the confidence interval. In a comment, report the estimated median survival time. What does this number mean in plain language?

  \item Estimate separate Kaplan-Meier curves by sex using \code{survfit()} and plot them with \code{ggplot2}. Hint: use \code{broom::tidy()} to convert the \code{survfit} object to a data frame, then plot with \code{geom\_step()} and \code{geom\_ribbon()} for confidence intervals. Save the plot as a PDF. Also run a log-rank test using \code{survdiff()}. In a comment, describe what you see: which group survives longer? Does the confidence interval for the two groups overlap? Report the log-rank test $p$-value and explain what it tests (whether the survival curves are statistically different).
