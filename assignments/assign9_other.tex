...
Load the required packages (\code{survival}, \code{survminer}, \code{marginaleffects}) and the \code{lung} dataset from \code{survival}.
...
\begin{enumerate}[label=\alph*)]
  ...
  \item Estimate separate Kaplan-Meier curves by sex using \code{survfit()} and plot them with \code{ggsurvplot()} from \code{survminer} (include confidence intervals, a $p$-value, and a risk table). Save the plot as a PDF. In a comment, describe what you see: which group survives longer? Does the confidence interval for the two groups overlap? The $p$-value shown is from the log-rank test --- report it and explain what it tests (whether the survival curves are statistically different).
